...
#pragma HLS interface ap_ctrl_none port=return
    ...
    float conv1[20*4*48];
    conv(crnn_conv1_w, crnn_conv1_b, input, conv1, 4, 10, 10, 49, 1, 48, true, 2, 2);
    float gru1[20*60];
    gru(crnn_gru1_gate_w, crnn_gru1_gate_b, crnn_gru1_cand_w, crnn_gru1_cand_b, conv1, gru1, 60, 192, 20);
    float gru2[20*60];
    gru(crnn_gru2_gate_w, crnn_gru2_gate_b, crnn_gru2_cand_w, crnn_gru2_cand_b, gru1, gru2, 60, 60, 20);
    float dense1[84];
...
void gru(const float* gate_weights, const float* gate_bias, const float* candidate_weights,
    const float* candidate_bias, float* data, float* output_data, int unit, int input, int length) {
    float state[unit];
